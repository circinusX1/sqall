...
void Button::setImage(int icon[4],const nanogui::Vector2i& s)
{
    ...
    if(icon[1]) mImage[1] = icon[1];
    if(icon[2]) mImage[2] = icon[2];
    if(icon[3]) mImage[3] = icon[3];
    ...
    if(icon[0])
    {
        ...
        vertices.col(2) << 0, 1;
        vertices.col(3) << 1, 1;
        mShader.init("ButtonViewShader", defaultImageViewVertexShader, defaultImageViewFragmentShader);
        mShader.bind();
...
void Button::draw(NVGcontext *ctx) {
    ...
    else if(mImage[0])
    {
        ...
        Vector2f scaleFactor = imageSizeF().cwiseQuotient(screenSize);
