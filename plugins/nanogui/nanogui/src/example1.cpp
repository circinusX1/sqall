...
public:
    ExampleApplication() : nanogui::Screen(Eigen::Vector2i(1024, 768), "NanoGUI Test") {
        ...
        auto imageView = new ImageView(imageWindow, t->texture());//mImagesData[0].first.texture());
        mCurrentImage = 0;
        ...
        */
        ...
        CheckBox *cb = new CheckBox(window, "Flag 1",
            [](CheckBox*,bool state) { cout << "Check box 1 state: " << state << endl; }
        );
        cb->setChecked(true);
        cb = new CheckBox(window, "Flag 2",
            [](CheckBox*,bool state) { cout << "Check box 2 state: " << state << endl; }
        );
